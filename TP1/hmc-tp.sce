...
function [y]=Filtering(B,prev,base)
    y = prev .* B(:,base) / (sum((B(:,base)) .* prev));
endfunction;

// Équations de lissage
function [y1]=Smooth1(y0,A,prev,filt)
  // Deuxième équation de lissage du lemme 5q
  y1 =  filt .* (A * (y0 ./ prev)) ;
endfunction;
...
//A = [0.9,0.1;0.1,0.9];
//B = [0.1,0.2,0.3,0.3;0.2,0.2,0.3,0.3];
A = [0.99,0.01;0.03,0.97];
B = [0.2697410,0.2084444,0.1983422,0.3234723;0.2463460,0.2475527,0.2982972,0.2078041];
pi = [0.25;0.75]
...
[r, a, b] = result(ADN,nb_etats,nb_iter,A, B, pi);


//tracer des proba de l'état +1 pour les nucléotides
//plot(r(1,:));
//tracer des proba de l'état -1 pour les nucléotides
//plot(r(2,:));
//evolution des coefficients diagonaux de a 
//plot(a(1,:));
//plot(a(2,:));
//evolution des coefficients de b
//plot(b');
//valeur finale des coefficients diagonaux de a et des coefficients de b
disp("valeur des coeff diagonaux de a");
disp(a(:,nb_iter));
disp("valeur f des coefficients de b");
disp(b(:,nb_iter));
